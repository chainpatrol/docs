...
description: "Reference for ChainPatrol asset type enums used in reports, checks, and lists: URL, PAGE, PUMP_FUN, ADDRESS, plus social, messaging, app store, and other platforms."
---
...
`URL`, `PAGE`, `PUMP_FUN`, and `ADDRESS` have dedicated pages because those pages already document normalization or enrichment. Every other type is listed with the same labels used in [`POST /threats/list`](/external-api/threats-list), [`POST /detection/list`](/external-api/detection-list), and [`chainpatrol asset types`](/cli/commands/asset#asset-types).
...
<CardGroup cols={2}>
  ...
  <Card title="PUMP_FUN" href="/glossary/asset-types/pump-fun">
    Pump.fun tokens, creator profiles, or platform pages. Preserves the case-sensitive Solana mint.
  </Card>
