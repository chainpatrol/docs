---
title: "Reports"
description: "How suspicious or safe assets get submitted to ChainPatrol with screenshots and context through the dashboard, API, or public Security Portal for review"
---

## Overview

A **Report** is how suspicious or safe activity gets submitted to ChainPatrol for review. Each report bundles together one or more assets, like websites, social profiles, apps, or blockchain addresses, along with the evidence needed to evaluate them.
...
Through the API, integrations or partners can submit threats programmatically for automated monitoring tools, partner integrations, custom workflows, and bulk submissions. See [Creating Reports](/external-api/creating-reports) for the full flow, including how to [upload evidence files](/external-api/attachment-upload) such as `.eml` messages and screenshots.
...
- **CLOSED** - Decisions have been made on all proposals in the report. The dashboard labels this status **Reviewed** to clarify that the proposals have been reviewed, not that takedowns have finished.

## Finding Your Reports

The **Reports page** in your dashboard shows all the reports for your organization, organized into four tabs:

- **ChainPatrol Review** - TODO reports waiting for ChainPatrol's team to triage
- **Customer Confirmation** - TODO reports escalated to your organization for approval. This is the same queue your Review page shows, and the same reports the [`needsCustomerReview` filter](/external-api/organization-reports-list) returns in the API and CLI.
- **In Progress** - Reports actively being reviewed
- **Reviewed** - Reports where every proposal has a decision

You can filter and search by creator, date, asset, and status to track the status of something you reported or follow up on threats flagged by your community. The filter menu also includes an **Attachments** section (**Has attachments** / **No attachments**) alongside the existing MX Records and Source filters, so you can narrow the list to reports that do or do not carry uploaded evidence. That is most useful for asset types whose content says little on its own, such as phone numbers and email addresses, where the attachment is the review signal. Each choice adds a removable chip to the active-filter strip and lives in the URL as `?hasAttachments=yes` or `?hasAttachments=no`, so a filtered view can be shared or bookmarked. The filter also narrows the per-tab counts and the CSV export, and it is available on the API as the `hasAttachments` parameter on [`GET /organization/reports`](/external-api/organization-reports-list). Exporting reports to CSV respects the tab that's currently active.
...
2. **Add Assets** - Paste in the assets you want to report, one per line: URLs (e.g., `https://fake-support-login.com`), social media profiles (e.g., `@fake_support`), blockchain addresses (e.g., `0x123...`), or any other asset type ChainPatrol monitors. The system automatically figures out what kind of asset each one is.
...
**After Review** - Status changes to CLOSED (shown as **Reviewed** in the dashboard), asset statuses are updated, you're notified of the outcome, and actions are taken (blocking, allowing, etc.).
...
- Report status tracks progress without micromanagement: TODO, IN_PROGRESS, and CLOSED (Reviewed) states provide visibility while letting the security team work without constant updates
