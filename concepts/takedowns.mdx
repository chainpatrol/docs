...
ChainPatrol staff can attach informational tags to assets to add context beyond the takedown lifecycle states. Tags appear as badges on the asset page, both in the Overview section and in the takedown timeline.

### Enforcement Exhausted

The first available tag is **Enforcement Exhausted**. It means all available enforcement options have been pursued with the relevant providers, and the asset remains active only due to provider inaction.

This distinguishes two situations that can look the same from the outside:

- **Not yet actioned** - The takedown is in the TODO or IN PROGRESS state and enforcement paths are still open
- **Enforcement Exhausted** - Every available enforcement path has been pursued, and removal now depends entirely on the providers

### Visibility

Tags are visible to members of the organization that blocked or allowlisted the asset. Visitors outside your organization do not see them.

### Filtering takedowns by tag

You can filter your takedowns list by asset tag. Open the filter menu on the takedowns table and select **Asset Tag**, then choose one or more tags such as Enforcement Exhausted. This is useful for isolating takedowns where all enforcement paths have already been pursued, so you can review them separately from takedowns that are still actionable.

<Note>
Tags are purely informational. They do not change takedown status, affect metrics, or trigger any automation.
</Note>
