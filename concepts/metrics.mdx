...
**Timelines and Speed** - Threats blocked per day, takedowns completed per day, median time to takedown, takedown success rate, trend analysis
...
**Takedown Success Rate** - The share of filed takedowns that ended in a removal, broken down by asset type. A takedown counts toward the period in which it was first submitted to a provider, and takedowns that were opened but never filed are excluded. Two rates are shown: the overall success rate, where still-open takedowns count against the result, and the resolved success rate, which only considers takedowns that reached an outcome. Available on the Metrics page and via the [takedown success rate API endpoint](/external-api/metrics-takedown-success-rate) when enabled for your organization.
...
**Takedowns** - Takedown records power Takedowns Filed, Takedowns Completed, Median time to takedown, and Takedown success rate per asset type and category. These metrics show how quickly and how reliably threats are being removed once identified.
