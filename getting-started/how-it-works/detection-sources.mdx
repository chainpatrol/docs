...
<Info>
ChainPatrol supports **30 different detection sources** across multiple categories
</Info>
...
Monitor public code hosting platforms for impersonation:

- **GitHub Search** - Searches GitHub for repositories, user accounts, issues, pull requests, and code that impersonate your brands. Runs hourly for P1 organizations and is enabled by default at that priority. Configurable options include `searchTypes` (defaults to `repositories` and `users`; `issues`, `pull_requests`, and `code` are opt-in) and `lookbackDays` (defaults to 7, applied to repository pushes, issue and pull request updates, and account creation dates). Queries are built from your organization's brand names and automatically exclude your protected GitHub owners via `-user:` qualifiers.
...
One category among several — specialized sources for cryptocurrency and blockchain threats, relevant to brands with a crypto or DeFi presence:
...
- **Yandex Image Search** - Reverse image searches your organization's brand imagery through Yandex Images to find impersonating profiles and storefronts reusing your logos
