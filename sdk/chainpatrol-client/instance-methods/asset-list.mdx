---
title: "asset.list"
description: Retrieve bulk lists of ChainPatrol-reviewed assets filtered by type, threat status, date range, and organization slug using the asset.list method.
---
...
<ParamField path="request" type="AssetListInput" required>
  ...
  <Expandable title="AssetListInput Properties">
    <ParamField path="type" type="enum<string>">
      The type of assets to retrieve. Omit the field entirely to sweep every asset type on a single cursor — each returned asset still carries its own `type`, so you can bucket the results client-side.

      Supported types include `URL`, `PAGE`, `ADDRESS`, and every other member of ChainPatrol's `AssetType` enum.

      An explicit `null` is rejected with a 400. Only omission opts into the full sweep, so a client that builds the request from a struct should leave the key out rather than setting it to `null`.
    </ParamField>
...
<ResponseField name="assets" type="array" required>
  Array of asset objects matching the filter criteria. A single response returns at most **1000** assets, whatever `per_page` asks for. Larger values are accepted but clamped, so keep following `next_page` until it comes back `null` rather than assuming one page holds everything.

  <Expandable title="Asset Object Properties">
    <ResponseField name="type" type="enum<string>" required>
      The asset's type (for example `URL`, `PAGE`, or `ADDRESS`). Present on every row so callers that omit the `type` filter can bucket the sweep client-side.
    </ResponseField>
...
<CardGroup cols={2}>
  <Card title="By Asset Type" icon="filter">
    Filter by `URL`, `PAGE`, `ADDRESS`, or any other member of the `AssetType` enum. Omit `type` entirely to sweep every asset type on one cursor.
  </Card>
...
```typescript Sweep Every Asset Type
// Omit `type` to mirror the full blocklist in one paginated sweep instead of
// looping over every member of the `AssetType` enum.
const result = await client.asset.list({
  status: "BLOCKED",
...
// Each returned asset still carries its own `type`, so you can bucket
// results client-side.
const byType = result.assets.reduce<Record<string, number>>((acc, asset) => {
