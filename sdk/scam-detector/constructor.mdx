...
To use the local mode, you need to instantiate the `ScamDetector` class with the `mode` option set to `local`.
...
To use the cloud mode, you need to instantiate the `ScamDetector` class with the `mode` option set to `cloud` and provide your API key.
...
<ParamField body="options" type="object" required>
  <Expandable title="properties" defaultOpen>
    <ParamField body="mode" type="'local' | 'cloud'" required>
      The `mode` property controls the mode of operation for the Scam Detector.
