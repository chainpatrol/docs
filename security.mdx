...
description: "How ChainPatrol protects customer data, systems, and services"
---
...
- Background checks (via **Checkr**) are performed for personnel with access to production systems, proportional to role and risk.
...
- Company-managed devices are enrolled in **Jamf** for device management and protected by **CrowdStrike** endpoint detection and response (EDR).
- Employee access to internal tools is authenticated via **Google SSO**, with network-level access to internal resources governed by **Cloudflare Zero Trust**.

## Infrastructure & Architecture

ChainPatrol operates a multi-tenant SaaS architecture hosted primarily on **AWS**, with additional use of **Vercel**, **Cloudflare**, and **Railway**.

- Workloads run in containerized form (ECS) within isolated VPCs.
...
- Core application data is stored in PlanetScale (AWS-hosted, managed MySQL); object storage uses Amazon S3 and Cloudflare R2.
- Network traffic is protected by Cloudflare's network security layer (including DDoS mitigation and WAF protections) in front of application infrastructure, in addition to AWS security groups and VPC-level network isolation.
...
Application secrets and credentials are managed through a dedicated secrets management vault, rather than stored in code, configuration files, or environment variables. Secrets are rotated on a defined 90-day cycle, and API keys are subject to a maximum 1-year expiration, after which they must be rotated.
...
- Access to production customer data specifically requires approval from our CTO.
...
- Vulnerabilities are prioritized and remediated according to documented SLAs based on severity (e.g., Critical: 30 days, High: 60 days).
...
- Security events and incidents are triaged and assigned a severity level, with defined remediation SLAs (e.g., Critical incidents within 7 days).
