...
richer context: status, associated reports, asset scans, takedown details, and which
platforms have consumed the block.
...
`BLOCKED` assets, with no date filter — narrow the window with `--from` and `--to`.
...
| `--per-page <n>` | Page size, 1–10000. Defaults to 100. A response returns at most 1000 assets regardless, so keep following `--next-page`. `--limit` is also accepted. |
