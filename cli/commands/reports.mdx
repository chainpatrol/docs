...
| `--attachment-url <u>` | Attachment URL. Repeatable. Takes a URL, not a file path — see the note below. |
...
<Note>
  `--attachment-url` takes a URL that already exists. To attach a local file — an `.eml`
  message, a screenshot, a PDF — upload it first with
  [`POST /attachment/upload`](/external-api/attachment-upload) and pass the URL it returns:

  ```bash
  URL=$(curl -s -X POST \
    "https://app.chainpatrol.io/api/v2/attachment/upload?fileName=evidence.eml" \
    -H "X-API-KEY: $CHAINPATROL_API_KEY" \
    --data-binary '@evidence.eml' | jq -r '.attachments[0].url')

  chainpatrol reports create --org acme --title "Phishing email" \
    --asset "https://phish.example:BLOCKED" --attachment-url "$URL"
  ```
</Note>
