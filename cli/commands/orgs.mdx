...
| `--subscription-status <l>` | Comma list: `PROSPECT`, `POC`, `ACTIVE`, `INTEGRATION`, `INACTIVE`. Defaults to the live statuses; pass `INACTIVE` explicitly to list churned organizations. |
...
Without `--subscription-status`, the listing returns only organizations in a live status
(`PROSPECT`, `POC`, `ACTIVE`, `INTEGRATION`). `INACTIVE` organizations never appear unless
you ask for them, so pass `--subscription-status INACTIVE` to audit churned accounts. The
filter does not widen access: you still only see organizations your credential can reach.
...
# churned orgs still running detection
chainpatrol orgs list --subscription-status INACTIVE --service-active detection
```
