...
<AccordionGroup>
  ...
  <Accordion title="Deno Deploy" icon="dinosaur">
    ...
    Go to the [Deno Deploy dashboard](https://dash.deno.com/projects/new) and click on **Create a new project**.

    ![Create a new Deno Deploy project](./deno-deploy-1.png)
  ...
  <Accordion title="Express.js" icon="js">

    If you're using [Express.js](https://expressjs.com/), you can create a new route to proxy requests to ChainPatrol.
    ...
    app.post('/api/chainpatrol', async (req, res) => {
        // Create a new request to the upstream server
        const upstreamRequest = new fetch.Request(UPSTREAM_URL, req)

        // Set the ChainPatrol API key
        const headers = new fetch.Headers(upstreamRequest.headers)
        headers.set('X-API-KEY', API_KEY)
        upstreamRequest.headers = headers
    ...
    You can now use the proxy server by sending requests to the URL of your Express.js route. For example, if your Express.js app is running on `http://localhost:3000`, you can send requests to `http://localhost:3000/api/chainpatrol`.

  </Accordion>

  <Accordion title="Next.js API Route" icon="react">
