...
Our goal is to prevent users from interacting with a malicious website that is intended to
steal user funds or credentials.
...
window.addEventListener("load", async function () {
  ...
  const response = fetch("https://app.chainpatrol.io/v2/asset/check", {
    method: "POST",
    headers: {
...
If the website is malicious, we should display a warning to the user. Our recommended
approach is to redirect the user to a warning page.
...
window.addEventListener("load", function () {
  ...
  if (response.ok) {
    ...
    if (data.status === "BLOCKED") {
      ...
      window.location.assign(`https://wallet.chainpatrol.com/warning?originUrl=${window.location.href}&reportUrl=https://wallet.chainpatrol.com/report`);
    } else {
      // The website is not malicious
    }
  }
