...
listening for the `DOMContentLoaded` event on the `window` object.
...
if (data.status === "BLOCKED") {
  ...
  window.location.assign("https://app.chainpatrol.io/warning");
}
```
...
window.location.assign(
  `https://app.chainpatrol.io/warning?originUrl=${window.location.href}`
);
```
