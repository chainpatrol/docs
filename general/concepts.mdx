...
Dark Web Monitoring lets organization owners and admins look up leaked credentials for their brand's tracked email assets, review masked results, acknowledge rows they have already checked, and get a Slack ping when leak volume for a monitored domain goes up.

It is an on-demand investigation tool, not a stored detections pipeline: search results live in the session, and ChainPatrol does not persist leak contents. It lives under **Employee Protection → Dark Web** and is independent of detection, blocklisting, and takedowns.

For more information, see the [Dark Web Monitoring](/concepts/dark-web-monitoring) concept page.