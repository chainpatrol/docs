...
Record your organization's decision on a proposal ChainPatrol escalated to you and you have not answered yet. These are exactly the proposals your Review page shows and the ones [`GET /organization/reports?needsCustomerReview=true`](/external-api/organization-reports-list) counts, so this endpoint lets you clear that queue without opening the web app.

<Note>
  Proposals on reports **your organization submitted itself** are not reviewable here and return `403`. ChainPatrol triages those, and staff approval blocks the asset on its own — submitting the report is already your organization's intent, so there is no second decision for you to record. You still see those reports in `GET /organization/reports`; they simply never enter your review queue.
</Note>
...
The credential is not a customer member of the organization, a safety check refused the write, or the proposal is not one that is waiting on you:
...
A proposal that is not in your review queue — because nobody escalated it to you, because you already answered, or because your organization submitted the report itself — returns:

```json
{
  "message": "Proposal 4321 is not waiting on your organization. Only proposals ChainPatrol escalated to you and you have not answered can be reviewed here.",
  "code": "FORBIDDEN"
}
```
...
- Answering an escalation takes the proposal out of your queue. Approving, rejecting, and escalating it back to ChainPatrol all count as answers; if ChainPatrol escalates it to you again afterwards, it returns to the queue.
- Obligatory Organization Admin Approval does not change which proposals appear here. It means an asset type will eventually need admin sign-off, not that an untriaged proposal is already waiting on you.
