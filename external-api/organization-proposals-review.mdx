...
The [ChainPatrol CLI](https://chainpatrol.com/docs/cli) wraps this endpoint as `chainpatrol proposals review`, and it composes directly with `chainpatrol reports list --needs-customer-review` — list what is waiting on you, then review each proposal by its ID:
...
| Flag | Description |
| --- | --- |
| `<proposalId>` or `--proposal-id <id>` | Proposal to review. From `proposals[].id` on the listing. |
| `--asset-id <id>` | Asset the proposal targets. From `proposals[].asset.id`. Must match, or the API returns `409`. |
| `--decision <d>` | `approve`, `reject`, or `watchlist`. Case-insensitive. |
| `--label <text>` | Block label. Required when approving. Quote multi-word labels. |
| `--brand-id <id>` | Brand the asset impersonates. Required with `--label "Brand Impersonation"` or `--label "Employee Impersonation"`. |
...
| `--yes` | Skip the confirmation prompt. Required outside an interactive terminal. |
| `--dry-run` | Print the payload without submitting it. |

### Confirmation

A review cannot be undone from the API, so the command asks for confirmation before writing:

- **Interactive terminal, no `--yes`.** Prints the payload and waits for `yes` on stdin. Anything else aborts.
- **`--yes`.** Writes immediately, no prompt.
- **`--json`, `--no-input`, or non-TTY stdin, without `--yes`.** Refuses with exit code `3` rather than blocking on a prompt nobody can see. Use `--yes` in CI.
- **`--dry-run`.** Prints the payload, submits nothing.

### Examples

```bash
# Approve with a general phishing label
chainpatrol proposals review 4321 --asset-id 987 --decision approve --label "General Phishing" --yes

# Approve as brand impersonation (brand ID required for this label)
chainpatrol proposals review 4321 --asset-id 987 --decision approve --label "Brand Impersonation" --brand-id 12 --yes

# Reject
chainpatrol proposals review 4321 --asset-id 987 --decision reject --reject-reason no_malicious_activity --yes

# Watchlist with a custom reason
chainpatrol proposals review 4321 --asset-id 987 --decision watchlist --watchlist-reason OTHER --note "Registrar suspended it" --yes

# Preview what would be sent, without submitting
chainpatrol proposals review 4321 --asset-id 987 --decision reject --reject-reason decayed --dry-run
```
