...
| needsCustomerReview | boolean | No | If true, only return reports with at least one proposal waiting on **your** review — the same queue your Review page shows. Use it to answer "what is waiting on me?", then act on the proposals with [`POST /organization/proposals/{proposalId}/review`](/external-api/organization-proposals-review). |
| threatActorIds | number[] | No | Filter to reports attributed to specific threat actors. Repeat the parameter for multiple IDs (`?threatActorIds=1&threatActorIds=2`). Get IDs from the `threatActor` field on reports you have already fetched. |
...
| reports[].threatActor      | object | The threat actor ChainPatrol attributed this report to, with `id` and `name`. `null` when the report has no attribution. Pass the `id` to `threatActorIds` to find related reports. |
...
| reports[].proposals[].id   | number | Unique identifier for the proposal. Pass it to [`POST /organization/proposals/{proposalId}/review`](/external-api/organization-proposals-review) to approve, reject, or watchlist the proposal. |
| reports[].proposals[].reviewStatus | string | Review state of the proposal (PENDING, APPROVED, REJECTED)        |
| reports[].proposals[].asset | object | The asset the proposal targets. Its `id` is the `assetId` the review endpoint requires. |
...
### Filter Reports Awaiting Your Review

Get the reports with at least one proposal waiting on your organization — the same queue your Review page shows:

```bash
curl -X GET 'https://app.chainpatrol.io/api/v2/organization/reports?needsCustomerReview=true&limit=20' \
  -H 'X-API-KEY: YOUR_API_KEY_HERE'
```

Each returned report carries the proposals to act on in `proposals[]`. Take `proposals[].id` and `proposals[].asset.id` from there and pass both to [`POST /organization/proposals/{proposalId}/review`](/external-api/organization-proposals-review).

### Filter by Threat Actor

Get all reports attributed to specific threat actors. ChainPatrol staff attribute reports to named threat actors during review; the actors you can filter by are those attributed to reports in your organization. Repeat the parameter to include multiple actors:

```bash
curl -X GET 'https://app.chainpatrol.io/api/v2/organization/reports?threatActorIds=42&threatActorIds=57' \
  -H 'X-API-KEY: YOUR_API_KEY_HERE'
```

To discover IDs, fetch reports without the filter and read the `threatActor` field on each report, or use the Threat Actor filter on the dashboard's Reports page.
