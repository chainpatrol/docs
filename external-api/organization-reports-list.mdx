...
| hasAttachments | boolean | No | Filter on evidence files uploaded with the report. Set to `true` to return only reports that carry at least one attachment; set to `false` to return only reports with none. Omit to return both. Soft-deleted attachments are ignored on both sides, so the two values are exact complements of each other. |
...
Get only reports that were submitted with evidence files (screenshots, `.eml` messages, etc.):

```bash
curl -X GET 'https://app.chainpatrol.io/api/v2/organization/reports?hasAttachments=true' \
  -H 'X-API-KEY: YOUR_API_KEY_HERE'
```

This is most useful for asset types where the content alone says little, such as phone numbers or email addresses, and the attachment is the review signal. Set `hasAttachments=false` to find the reports with none.
