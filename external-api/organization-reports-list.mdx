...
| needsCustomerReview | boolean | No | If true, only return reports the organization still has to action (matches the Review page). If false, exclude them. |
...
Use `needsCustomerReview=true` to return only reports the organization still has to action — the same queue the organization's Review page shows. This is useful for building a "reports needing your review" dashboard or exporting the current approval backlog. Pass `false` to return only reports that are not waiting on the organization.
...
Obligatory Organization Admin Approval does not widen this filter — an untriaged proposal is still ChainPatrol's to review, and only reaches the organization once staff escalate it.
