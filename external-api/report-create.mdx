...
`attachmentUrls` accepts URLs to any supported evidence file, not just images. Screenshots, `.eml` phishing emails, and PDFs all work.

If your evidence is a file rather than an already-hosted URL, upload it to ChainPatrol first with [Upload Attachment](/external-api/attachment-upload), then pass the returned URL in `attachmentUrls`. Files uploaded with an organization API key are stored privately and are only visible to signed-in members of your organization and ChainPatrol staff.
