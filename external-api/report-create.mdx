...
New to this endpoint? Start with [Creating Reports](/external-api/creating-reports) — a
step-by-step walkthrough of the whole flow, with worked examples.

## Attachments

`attachmentUrls` accepts URLs of any evidence file, not just images — `.eml` and `.msg`
messages, PDFs, documents, and screenshots all work. Images are displayed inline on the
report; other files are listed as downloadable evidence that our takedown team can forward
to a registrar or hosting provider.

To send us the file itself rather than a link you host, upload it first with
[Upload Attachment](/external-api/attachment-upload) and pass the URL it returns:

```bash
# 1. upload the file, get a URL back
curl -X POST 'https://app.chainpatrol.io/api/v2/attachment/upload?fileName=evidence.eml' \
  -H 'X-API-KEY: YOUR_API_KEY_HERE' \
  --data-binary '@evidence.eml'

# 2. reference that URL here, in attachmentUrls
```

Attachments belong to the report as a whole rather than to an individual asset.
...
Already-reported assets do not fail a mixed batch. When a submission contains at least one new, valid asset, ChainPatrol treats assets with `ASSET_ALREADY_CORRECT`, `ALREADY_PENDING_REVIEW`, or `REPORT_ALREADY_EXISTS` outcomes as no-ops and creates the report with the remaining assets. A `422` with these error types is returned only when every asset in the batch is a no-op. Validation failures such as `INVALID_FORMAT` or `DUPLICATE_ASSETS` still reject the whole batch.
...
| `ASSET_ALREADY_CORRECT`  | Asset already has the requested status                | No action needed - skipped in mixed batches; asset is already in desired state |
| `ALREADY_PENDING_REVIEW` | Asset has a pending proposal for the requested status | No action needed - skipped in mixed batches; wait for existing proposal to be processed |
| `REPORT_ALREADY_EXISTS`  | A pending report/proposal for this asset exists       | No action needed - skipped in mixed batches; check the existing report in your dashboard |
...
In a mixed batch the asset is skipped and the report is created with the remaining assets; this error is only returned when every asset in the batch is a no-op
...
Validation failures reject the whole batch; already-reported assets are skipped when the batch contains other valid assets
