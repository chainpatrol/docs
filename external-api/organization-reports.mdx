...
Set `needsCustomerReview` to `true` to return only reports the organization's own admins still have to approve, or `false` to return only reports that are not waiting on them. Use this when you want to surface the customer approval backlog — for example, to power a "reports needing your review" dashboard or export.
...
Reports the organization submitted itself are never included, because staff can action those without customer sign-off.
