...
<Note>
Omitting `type` matches far more rows per page, so prefer a larger `per_page`
(up to the 10000 maximum) and keep following `next_page` until it comes back
`null`. Responses are still capped at 4MB — if you hit that limit, lower
`per_page`.
</Note>
