...
| include   | array  | No       | Subset of metrics to compute. Allowed values: `reports`, `newThreats`, `threatsWatchlisted`, `takedownsFiled`, `takedownsInProgress`, `takedownsCompleted`, `takedownsCancelled`, `domainThreats`, `twitterThreats`, `telegramThreats`, `otherThreats`, `blockedByType`, `blockedByDay`. Omit for all metrics. Fields not in `include` come back as `null`. |
...
```json
{
  "metrics": {
    ...
    "takedownsFiled": 45,
    "takedownsInProgress": 8,
    "takedownsCompleted": 32,
    "takedownsCancelled": 5,
    "domainThreats": 120,
    "twitterThreats": 40,
    "telegramThreats": 25,
    "otherThreats": 60
...
| metrics.takedownsInProgress    | number | Number of takedowns currently in progress                  |
| metrics.takedownsCompleted     | number | Total number of takedowns successfully completed           |
| metrics.takedownsCancelled     | number | Number of takedowns currently cancelled                    |
...
- `takedownsInProgress` and `takedownsCancelled` count takedowns currently in that status whose `updatedAt` falls in the requested window (same windowing as `takedownsFiled`), so they reconcile with the Takedowns list filtered by status over the same date range
