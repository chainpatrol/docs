...
icon: webhook
title: Webhooks
description: Use ChainPatrol webhooks to receive real-time asset status and blocklist updates, verify signatures, and keep your wallet, app, or SecOps pipeline in sync.
---
...
This event is sent when an asset's status is escalated, typically through a proposal process.

```json
{
  "type": "asset.status-escalated",
  "created_at": "2025-01-15T10:30:00.000Z",
  "data": {
    "asset_id": 12345,
    "content": "https://example.com/phishing",
    "type": "url",
    "status": "BLOCKED",
    "proposal_id": 67890,
    "organization_id": 100
...
3. Select which events you want to receive (available events include `asset.status-updated`, `asset.status-escalated`, and `organization.threat-detection-added`)
4. Optional HTTP headers for supported integrations (currently Google SecOps headers are available)
...
ChainPatrol supports specific hardcoded HTTP headers that can be configured for webhook requests. Currently, the following headers are available for Google SecOps integration:

- `X-goog-api-key`: Your Google API key
- `X-Webhook-Access-Key`: Your webhook access key

When configuring a webhook for Google SecOps, you can provide values for these headers, and they will be automatically included in all webhook requests sent to your endpoint, allowing Google SecOps to authenticate and process the webhook events.
